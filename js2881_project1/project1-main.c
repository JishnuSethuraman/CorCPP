...
imatrix* add(imatrix* m1, imatrix* m2){
    ...
    if(m1->width == m2->width && m1->height == m2->height){
        imatrix* sum = init_blank_rgb_image(m1->width , m1->height, 3);
        for(int i = 0;i < m1->height;i++){
            for(int j = 0;j< m1->width;j++){
                sum->r[i][j] = m1->r[i][j] + m2->r[i][j];
                sum->g[i][j] = m1->g[i][j] + m2->g[i][j];
                sum->b[i][j] = m1->b[i][j] + m2->b[i][j];
                if(sum->r[i][j]>255){
                    sum->r[i][j]=255;
                }
                if(sum->g[i][j]>255){
...
imatrix* subtract(imatrix* m1, imatrix* m2){
    ...
    if(m1->width == m2->width && m1->height == m2->height){
        imatrix* diff = init_blank_rgb_image(m1->width , m1->height, 3);
        for(int i = 0;i < m1->height;i++){
            for(int j = 0;j< m1->width;j++){
                diff->r[i][j] = m1->r[i][j] - m2->r[i][j];
                diff->g[i][j] = m1->g[i][j] - m2->g[i][j];
                diff->b[i][j] = m1->b[i][j] - m2->b[i][j];
                if(diff->r[i][j]<0){
                    diff->r[i][j]=0;
                }
                if(diff->g[i][j]<0){
...
imatrix* multiply(imatrix* m1, imatrix* m2){
    ...
    if(m1->width == m2->height){
        imatrix* p = init_blank_rgb_image(m1->width , m1->height, 3);
        for(int i = 0;i < m1->height;i++){
            for(int j = 0;j < m2->width;j++){
                ...
                for(int k = 0;k < m1->width; k++){
                    p->r[i][j] = m1->r[i][k] * m2->r[k][j];
                    p->g[i][j] = m1->g[i][k] * m2->g[k][j];
                    p->b[i][j] = m1->b[i][k] * m2->b[k][j];
